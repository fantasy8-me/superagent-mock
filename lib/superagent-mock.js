...
function mock (superagent, config) {
  ...
  function testUrlForPatterns(method, url) {
    ...
    var match = config.filter(function (parser) {
      return (!parser.method || method === parser.method) && new RegExp(parser.pattern, 'g').test(url);
    })[0] || null;
  ...
  Request.prototype.end = function (fn) {
    ...
    if (parser) {
      ...
      try {
        var fixtures = parser.fixtures(match, this.params, this.method);
        fn(null, parser.callback ? parser.callback(match, fixtures) : { body: fixtures });
      } catch(err) {
        fn(err, undefined);
      }
    } else {
